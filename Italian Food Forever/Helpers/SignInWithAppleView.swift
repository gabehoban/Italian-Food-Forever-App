		...
		func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
			...
			if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
				...
				guard let appleIDToken = appleIDCredential.identityToken else {
					utils().LOG(error: "Unable to fetch identity token", value: "", title: "SignInWithApple authController")
					parent.stopActivityIndicator()
					parent.presentAlert(title: "Error", message: "Unable to fetch identity token")
					return
				}
				guard let idTokenString = String(data: appleIDToken, encoding: .utf8) else {
					utils().LOG(error: "Unable to serialize token string from data: \(appleIDToken.debugDescription)", value: "", title: "SignInWithApple authController")
					parent.stopActivityIndicator()
					...
					case .success(let authDataResult):
						...
						SparkAuth.handle(signInWithAppleResult) { result in
							...
							case .failure(let err):
								utils().LOG(error: err.localizedDescription, value: "", title: "SignInWithApple authController")
								parent.stopActivityIndicator()
